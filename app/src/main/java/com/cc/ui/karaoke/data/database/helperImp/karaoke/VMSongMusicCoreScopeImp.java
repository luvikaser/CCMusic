 ...
 */
public class VMSongMusicCoreScopeImp extends VMBaseSongKaraokeScopeImp implements VMSongKaraokeScope {
    ...
    public Cursor getCursor() {
        ...
        Cursor cursor = cr.query(VMSongMusicCoreTable.getInstance().getContentUri(), null, selection, selectionArg,
                sortOrder);
    ...
    public Cursor searchQuery(String key, String volId) {
        String selection = String.format(VMSongVirtualCoreMusicTable.SELECTION_SEARCH,
        ...
        Cursor cursor = contentResolver.query(VMSongMusicCoreTable.getInstance().getContentUri(), null, selection,
                selectionArgs, null);
        if (cursor != null && cursor.moveToFirst()) {
            return cursor;
        }
    ...
    public Cursor getCursorWithVolsName(String volsName, String language) {
        ...
        ContentResolver cr = MusicApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = cr.query(VMSongMusicCoreTable.getInstance().getContentUri(), null, selection,
                selectionArg,
                sortOrder);
        if (cursor != null && cursor.moveToFirst()) {
            return cursor;
