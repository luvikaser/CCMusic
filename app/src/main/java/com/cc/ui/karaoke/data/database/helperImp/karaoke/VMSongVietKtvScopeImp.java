 ...
 */
public class VMSongVietKtvScopeImp extends VMBaseSongKaraokeScopeImp implements VMSongKaraokeScope {
    @Override
    public Cursor getCursor() {
        ...
        Cursor cursor = cr.query(VMSongVietKtvTable.getInstance().getContentUri(), null, selection, selectionArg,
                sortOrder);
    ...
    public Cursor searchQuery(String key, String volId) {
        String selection = String.format(VMSongVirtualVietKtvTable.SELECTION_SEARCH,
                DatabaseUtils.sqlEscapeString(key + "*"));
        Log.d(TAG, "searchQuery = " + selection);
        ContentResolver contentResolver = MusicApplication.getInstance().getApplicationContext().getContentResolver();

        String[] selectionArgs = null;
        String sortOrder = VMSongArirangTable.SNAME_COLUMN + " ASC";

        Cursor cursor = contentResolver.query(VMSongVietKtvTable.getInstance().getContentUri(), null, selection, null, null);
        if (cursor != null && cursor.moveToFirst()) {
            return cursor;
        }
    ...
    public Cursor getCursorWithVolsName(String volsName, String language) {
        ...
        ContentResolver cr = MusicApplication.getInstance().getApplicationContext().getContentResolver();
        Cursor cursor = cr.query(VMSongVietKtvTable.getInstance().getContentUri(), null, selection,
                selectionArg,
                sortOrder);
        if (cursor != null && cursor.moveToFirst()) {
            return cursor;
