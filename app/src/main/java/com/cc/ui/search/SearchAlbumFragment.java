 ...
 * Since: 11/26/2016.
 */
public class SearchAlbumFragment extends BaseFragmentSearch implements SongLocalAdapter
    ...
    void onClickSearchMore(View v) {
        mPresenter.searchAlbum(searchText);
    }
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        if (TextUtils.isEmpty(newText)) {
            Log.e(TAG, "isEmpty");
            hideSearchMore();
            mAlbumsAdapter.edit().replaceAll(new ArrayList<MediaBrowserCompat.MediaItem>()).commit();
            return false;
        }
        ...
        mAlbumsAdapter.edit().replaceAll(filteredModelList).commit();
    ...
    protected void onMediaBrowserChildrenLoaded(@NonNull String parentId, List<MediaBrowserCompat.MediaItem> children) {
        try {
            ...
            mAlbumsAdapter.edit().removeAll().commit();
            mAlbumsAdapter.edit().replaceAll(mMediaItemList);
        } catch (Throwable t) {
    ...
    public void songSearchComplete() {
        ...
        MusicService.setMusicTypeCurrent(MusicEnumApp.MusicType.ALBUM_ONLINE);
