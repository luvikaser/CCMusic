...
import com.cc.domain.utils.ResourceHelper;
import com.cc.ui.playback.PlaybackFullScreenActivity;
...
public class MediaNotificationManager extends BroadcastReceiver {
    ...
    private PendingIntent createContentIntent(MediaMetadataCompat mMetadata) {
        Intent openUI = new Intent(mService, PlaybackFullScreenActivity.class);
        openUI.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        openUI.putExtra(BaseMediaActivity.EXTRA_START_FULLSCREEN, true);
        if (mMetadata != null) {
            openUI.putExtra(BaseMediaActivity.EXTRA_CURRENT_MEDIA_DESCRIPTION, mMetadata);
        }
    ...
    private Notification createNotification() {
        ...
        String whereVal[] = {description.getTitle().toString()};
        Cursor cursor = null;
        Bitmap artwork = null;
        try{
            cursor = mService.getBaseContext().getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, columns, where, whereVal, null);

            if (cursor.moveToFirst()){
                long id = cursor.getLong(7);

                artwork = ImageLoader.getInstance().loadImageSync(CCMusicUtils.getAlbumArtUri(id).toString());
                if (artwork == null) {
                    artwork = ImageLoader.getInstance().loadImageSync("drawable://" + R.drawable.ic_album_default);
                }
            }
        } finally {
            if (cursor != null)
